...
impl L402Module {
    ...
    pub async fn verify_cashu_token(&self, token: &str, amount_msat: i64, lnurl_address: Option<String>) -> Result<bool, String> {
        ...
        if cashu::is_p2pk_mode_enabled() {
            ...
            cashu::verify_cashu_token_p2pk(token, amount_msat, lnurl_address).await
        } else {
            info!("💰 Using standard Cashu verification (with mint receive)");
            cashu::verify_cashu_token(token, amount_msat, lnurl_address).await
        }
    }
...
pub static mut NGX_HTTP_L402_COMMANDS: [ngx_command_t; 5] = [
    ...
    ngx_command_t {
        name: ngx_string!("l402_lnurl_address"),
        type_: (NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1) as ngx_uint_t,
...
impl Merge for ModuleConfig {
    fn merge(&mut self, prev: &ModuleConfig) -> Result<(), MergeConfigError> {
        ...
        if self.lnurl_address.is_none() && prev.lnurl_address.is_some() {
            self.lnurl_address = prev.lnurl_address.clone();
        }
        Ok(())
    }
}
...
pub unsafe extern "C" fn l402_access_handler_wrapper(request: *mut ngx_http_request_t) -> isize {
    ...
    let (auth_header, uri, method, amount_msat, macaroon_timeout, lnurl_address) = unsafe {
        let r = &mut *request;
        ...
        let lnurl_address = conf.lnurl_address.clone();
    ...
    let result = l402_access_handler(auth_header, uri, method, final_amount, caveats.clone(), lnurl_address);
...
pub fn l402_access_handler(
    ...
    if let Some(auth_str) = auth_header {
        ...
        if auth_str.starts_with("Cashu ") {
            ...
            let verify_result =
                rt.block_on(async { module.verify_cashu_token(&token, amount_msat, lnurl_address.clone()).await });
...
pub unsafe extern "C" fn ngx_http_l402_lnurl_set(
    ...
    unsafe {
        ...
        if val.trim().is_empty() {
            error!("❌ Invalid lnurl_address configuration value: empty string");
            return std::ptr::null_mut();
        }

        conf.lnurl_address = Some(val.to_string());
        info!("⚙️ Set L402 LNURL address to: {}", val);
    };
