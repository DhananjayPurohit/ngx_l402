...
/// Render the self-contained HTML 402 payment page.
///
/// All values are injected server-side so the page works with no external
/// network requests (the QR code is an inline SVG generated by the
/// `qrcode-generator` crate).
fn render_payment_page(
    ...
    .unwrap_or_else(|_| "<svg xmlns='http://www.w3.org/2000/svg' width='300' height='300'><rect width='300' height='300' fill='#1a1a2e'/><text x='150' y='150' text-anchor='middle' fill='#8b5cf6' font-size='12'>QR Error</text></svg>".to_string());

    let amount_sats = amount_msat / 1000;
    let invoice_short = if invoice.len() > 40 {
        format!("{}…{}", &invoice[..20], &invoice[invoice.len()-10..])
    } else {
        invoice.to_string()
    };
    ...
    let cashu_tab_html = if cashu_enabled {
        let payment_req_hint = cashu_payment_request
            .map(|r| format!("<p class='payment-req'>Payment request: <code>{}</code></p>", &r[..r.len().min(60)]))
            .unwrap_or_default();
        format!(r#"<div id="tab-ecash" class="tab-panel hidden">
          <div class="card">
            {payment_req_hint}
            <textarea id="cashu-token" placeholder="Paste your Cashu token here (cashuA...)" rows="4"></textarea>
            <button class="btn btn-primary" onclick="submitCashu()">⚡ Submit Token</button>
    ...
    let ecash_tab_btn = if cashu_enabled {
        r#"<button class="tab-btn" id="tab-btn-ecash" onclick="switchTab('ecash')">ECASH</button>"#
    } else {
        ""
    };
    ...
    function startPolling() {{
        ...
        fetch(window.location.href, {{
            ...
            if (r.ok || r.status === 200) {{
                document.getElementById('auto-status').innerHTML = '✅ Payment confirmed! Redirecting…';
...
"#, mac = macaroon_b64)
    ...
    let auto_detect_section = if auto_detect {
        r#"<div id="auto-status" class="auto-status">
            <div class="spinner"></div>
            <span>Waiting for payment confirmation…</span>
            <button class="btn-link" onclick="document.getElementById('auto-status').classList.add('hidden');document.getElementById('preimage-section').classList.remove('hidden')">Enter preimage manually</button>
          </div>"#
    } else {
        ""
    };
...
<style>
  ...
  *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0}}
  :root{{
    --bg:#07070f;--surface:#0e0e1c;--surface2:#15152a;--border:#1e1e3a;
    --accent:#7c3aed;--accent2:#a855f7;--accent-glow:rgba(124,58,237,.35);
    --text:#e2e2f0;--text-muted:#6b6b8a;--text-dim:#3a3a5c;
  ...
  body{{display:flex;align-items:center;justify-content:center;min-height:100vh;padding:1.5rem;
    background-image:radial-gradient(ellipse 80% 60% at 50% -20%,rgba(124,58,237,.18),transparent),
                     radial-gradient(ellipse 50% 40% at 80% 80%,rgba(168,85,247,.08),transparent)}}
  .container{{width:100%;max-width:480px;display:flex;flex-direction:column;gap:1.25rem}}
  /* Header */
  .header{{text-align:center}}
  .badge{{display:inline-flex;align-items:center;gap:.4rem;background:rgba(239,68,68,.12);
    border:1px solid rgba(239,68,68,.3);border-radius:9999px;padding:.25rem .75rem;
    font-size:.75rem;font-weight:600;letter-spacing:.08em;color:#f87171;margin-bottom:.75rem}}
  .header h1{{font-size:1.6rem;font-weight:700;letter-spacing:-.02em;
    background:linear-gradient(135deg,#e2e2f0 0%,#a855f7 100%);
    -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}}
  .header p{{color:var(--text-muted);font-size:.9rem;margin-top:.35rem}}
  .amount{{display:inline-flex;align-items:baseline;gap:.3rem;margin-top:.5rem;
    background:linear-gradient(135deg,rgba(124,58,237,.15),rgba(168,85,247,.08));
    border:1px solid rgba(124,58,237,.3);border-radius:.75rem;padding:.4rem 1rem}}
  .amount-num{{font-size:1.5rem;font-weight:700;color:#a855f7}}
  .amount-unit{{font-size:.8rem;color:var(--text-muted);font-weight:500}}
  /* Tabs */
  .tabs{{display:flex;background:var(--surface);border:1px solid var(--border);border-radius:.75rem;padding:.25rem;gap:.25rem}}
  .tab-btn{{flex:1;padding:.5rem;border:none;border-radius:.5rem;background:transparent;
    color:var(--text-muted);font-size:.78rem;font-weight:600;letter-spacing:.06em;cursor:pointer;transition:all .2s}}
  ...
  /* Cards */
  .card{{background:rgba(14,14,28,.8);border:1px solid var(--border);border-radius:1rem;
    padding:1.25rem;backdrop-filter:blur(12px);display:flex;flex-direction:column;gap:1rem}}
  .tab-panel{{display:flex;flex-direction:column;gap:1rem}}
  .tab-panel.hidden{{display:none}}
  /* QR */
  .qr-wrap{{display:flex;justify-content:center;align-items:center;
    background:var(--surface2);border:1px solid var(--border);border-radius:.75rem;padding:1rem}}
  .qr-wrap svg{{border-radius:.5rem;width:240px;height:240px}}
  /* Invoice */
  .invoice-box{{background:var(--surface2);border:1px solid var(--border);border-radius:.6rem;padding:.65rem .85rem;
    display:flex;align-items:center;gap:.6rem;cursor:pointer;transition:border-color .2s}}
  .invoice-box:hover{{border-color:var(--accent2)}}
  .invoice-text{{font-family:'JetBrains Mono',monospace;font-size:.72rem;color:var(--text-muted);flex:1;word-break:break-all;overflow:hidden;
  ...
  /* Auto-detect */
  .auto-status{{display:flex;flex-direction:column;align-items:center;gap:.6rem;padding:.75rem;
    background:rgba(124,58,237,.07);border:1px solid rgba(124,58,237,.2);border-radius:.75rem;text-align:center}}
  .auto-status span{{font-size:.88rem;color:var(--text-muted)}}
  .spinner{{width:20px;height:20px;border:2.5px solid rgba(124,58,237,.2);
    border-top-color:var(--accent2);border-radius:50%;animation:spin .8s linear infinite}}
  @keyframes spin{{to{{transform:rotate(360deg)}}}}
  .btn-link{{background:none;border:none;color:var(--text-dim);font-size:.75rem;cursor:pointer;
    text-decoration:underline;padding:0;margin-top:.25rem;transition:color .2s}}
  .btn-link:hover{{color:var(--accent2)}}
  /* Forms */
  label{{font-size:.8rem;font-weight:500;color:var(--text-muted);letter-spacing:.03em}}
  input,textarea{{width:100%;background:var(--surface2);border:1px solid var(--border);border-radius:.6rem;
    padding:.65rem .85rem;color:var(--text);font-family:'JetBrains Mono',monospace;font-size:.8rem;
  ...
  /* Buttons */
  .btn{{display:flex;align-items:center;justify-content:center;gap:.4rem;width:100%;
    padding:.7rem 1.25rem;border:none;border-radius:.7rem;font-weight:600;font-size:.88rem;
  ...
  /* Misc */
  .payment-req{{font-size:.75rem;color:var(--text-dim);word-break:break-all}}
  .payment-req code{{color:var(--text-muted)}}
  .error-msg{{background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.25);border-radius:.5rem;
    padding:.5rem .75rem;font-size:.8rem;color:#f87171}}
  .error-msg.hidden{{display:none}}
  .divider{{border:none;border-top:1px solid var(--border);margin:.25rem 0}}
  .footer{{text-align:center;font-size:.72rem;color:var(--text-dim)}}
...
<div class="container">
  ...
  <div class="footer">
    Secured by <a href="https://github.com/getAlby/ngx_l402" target="_blank" rel="noopener">ngx_l402</a> · L402 Protocol
  </div>
</div>
        ...
        match header_result {
            Some(header_value) => {
                ...
                if let Some((macaroon_b64, invoice)) = parse_l402_header_value(&header_value) {
                    let cashu_en = cashu_ecash_support && p2pk_mode;
